...
from web3py_ext.klaytn_account.instrumented_local_account import (
    TYPED_TRANSACTION_DECODE_FORMATTERS,
)
...
class FeeDelegatedSmartContractExecutionTransaction(_TypedTransactionImplementation):
    """
    ...
    @classmethod
    def from_dict(cls, dictionary: Dict[str, Any]) -> "FeeDelegatedSmartContractExecutionTransaction":
        """
        Builds a FeeDelegatedSmartContractExecutionTransaction from a dictionary.
        Verifies that the dictionary is well formed.
        """
        # convert dynamic gas price field to klaytn contract execution transaction type
        if 'maxFeePerGas' in dictionary:
            gasPrice = dictionary['maxFeePerGas']
            dictionary = dissoc(dictionary, 'maxFeePerGas', 'maxPriorityFeePerGas')
            dictionary = assoc(dictionary, 'gasPrice', gasPrice)
    ...
    @classmethod
    def from_bytes(cls, encoded_transaction: HexBytes) -> "FeeDelegatedSmartContractExecutionTransaction":
        """Builds a FeeDelegatedSmartContractExecutionTransaction from a signed encoded transaction."""
    ...
    def hash(self) -> bytes:
        """
        https://docs.klaytn.foundation/content/klaytn/design/transactions/basic#rlp-encoding-for-sendertxhash-4
        """
    ...
    def feepayer_payload(self) -> bytes:
        ...
        Here, the TransactionPayload = rlp([nonce, gasPrice, gas, to, value, from, input, humanReadable, codeFormat, txSignatures, feePayer, feePayerSignatures])
    ...
    def payload(self) -> bytes:
        """
        Returns this transaction's payload as bytes.
        Here, the TransactionPayload = rlp([nonce, gasPrice, gas, to, value, from, input, txSignatures])
        txSignatures = [[v, r, s], ...]
        """
