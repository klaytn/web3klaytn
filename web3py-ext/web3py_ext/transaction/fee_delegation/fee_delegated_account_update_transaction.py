...
class FeeDelegatedAccountUpdateTransaction(_TypedTransactionImplementation):
    ...
    @classmethod
    def from_dict(cls, dictionary: Dict[str, Any]) -> "FeeDelegatedAccountUpdateTransaction":
        """
        Builds a FeeDelegatedAccountUpdateTransaction from a dictionary.
    ...
    @classmethod
    def from_bytes(cls, encoded_transaction: HexBytes) -> "FeeDelegatedAccountUpdateTransaction":
        """Builds a FeeDelegatedAccountUpdateTransaction from a signed encoded transaction."""
    ...
    def hash(self) -> bytes:
        ...
        transaction_without_signature_fields = dissoc(transaction_with_encoded_key, "signatures", "feePayerSignatures", "chainId", "feePayer")
    ...
    def feepayer_payload(self) -> bytes:
        ...
        vrs = {"v":self.dictionary["v"],"r":self.dictionary["r"],"s":self.dictionary["s"]}
        transaction_without_signature_fields = dissoc(transaction_with_encoded_key, "v", "r", "s", "type", "chainId")
    ...
    def feepayer_vrs(self) -> Tuple[int, int, int]:
        """Returns (v, r, s) if they exist."""
        if 'signatures' not in self.dictionary:
            raise ValueError("attempting to encode an unsigned transaction")
        if len(self.dictionary['feePayerSignatures']) > 1:
            raise TypeError("attemping to only 1 vrs about multi-signed transaction. use klaytn_recover_transaction")
        signature = self.dictionary['feePayerSignatures'][0]
        if not all(k in signature for k in "vrs"):
            raise ValueError("attempting to encode an unsigned transaction")
        return (signature["v"], signature["r"], signature["s"])
